...
function mergeStudentRecords(incomingStudents, options = {}) {
    ...
    const mergedStudents = incomingList.map(student => {
        ...
        return {
            ...
            remainingAmount: student.remainingAmount ?? localStudent.remainingAmount ?? '',
            monthlyFeeCustom: student.monthlyFeeCustom ?? localStudent.monthlyFeeCustom ?? false,
            freeStudy: student.freeStudy ?? localStudent.freeStudy ?? false,
            zeroFeeReason: student.zeroFeeReason || localStudent.zeroFeeReason || '',
            guardianName: student.guardianName || localStudent.guardianName || '',
            guardianContact: student.guardianContact || localStudent.guardianContact || '',
            address: student.address || localStudent.address || '',
...
function getEmailSchoolName() {
    try {
        ...
        return String(branding.schoolName || branding.schoolTitle || 'Kids Roots Jand').trim() || 'Kids Roots Jand';
    } catch (_error) {
        return 'Kids Roots Jand';
    }
}
